...
public class BoardStompClients {
  ...
  private void setupClient(Map<String, String> token)
    ...
    session.subscribe("/user/queue/errors", new GameErrorStompFrameHandler(token.get("localId")));
  }
  ...
  public String getMessage(int userIndex, long timeout) {
    ...
    await()
        ...
        .until(
            () -> {
              String m =
                  boardMessages.get(users.get(userIndex).getUid()).poll(100, TimeUnit.MILLISECONDS);
              message.set(m);
              return m != null;
            });
    return message.get();
